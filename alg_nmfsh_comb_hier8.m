function [clusters, objs, Ws, priorities] = alg_nmfsh_comb_hier8(X, k, WHinit, random_run, trial_allowance, vec_norm, normW, conv, scoring, computeobj, anls_alg, tol, maxiter)
...
for run = 1 : random_run
	[temp_W, temp_H] = nmfsh_comb_rank2(X_subset, 2, WHinit{1, 1, run}.Winit(term_subset, 2*(init_used-1)+1:2*init_used), WHinit{1, 1, run}.Hinit(2*(init_used-1)+1:2*init_used, :), vec_norm, normW, conv, tol, maxiter, anls_alg);
	if computeobj
		temp_obj = obj_compute(X_subset, temp_W, temp_H);
	else
		temp_obj = 0;
	end
	if temp_obj < obj
...
for i = 1 : k-1
	...
	if i > 1
		...
		Hs{i}(end-num_cols_move+1:end, :) = Hs{i}(end-num_cols_move:end-1, :);
	end
	Ws{i}(:, label_pos) = W_buffer{unique_cluster_pos}(:, 1);
	Hs{i}(label_pos, :) = H_buffer{unique_cluster_pos}(1, :);
	Ws{i}(:, label_split_pos) = W_buffer{unique_cluster_pos}(:, 2);
	Hs{i}(label_split_pos, :) = H_buffer{unique_cluster_pos}(2, :);
	if computeobj
		H = nnlsm_blockpivot(Ws{i}, X, 0, Hs{i});
		objs(i) = obj_compute(X, Ws{i}, H);
	else
		objs(i) = 0;
	end
	W_buffer{end+1} = [];
	...
	H_buffer{end+1} = [];
	H_buffer(end-num_cols_move+1:end) = H_buffer(end-num_cols_move:end-1);
	priority(end+1) = 0;
	priority(end-num_cols_move+1:end) = priority(end-num_cols_move:end-1);

	subset = find(cluster == label);
	init_used = init_used + 1;
	[cluster, W_buffer_one, H_buffer_one, priority_one] = trial_split(cluster, trial_allowance, unbalanced, priority, X, subset, label, Ws{i}(:, label_pos), scoring, WHinit, random_run, init_used, vec_norm, normW, conv, tol, maxiter, computeobj, anls_alg);
	W_buffer{label_pos} = W_buffer_one;
	H_buffer{label_pos} = H_buffer_one;
	priority(label_pos) = priority_one;

	subset = find(cluster == label+split);
	init_used = init_used + 1;
	[cluster, W_buffer_one, H_buffer_one, priority_one] = trial_split(cluster, trial_allowance, unbalanced, priority, X, subset, label+split, Ws{i}(:, label_split_pos), scoring, WHinit, random_run, init_used, vec_norm, normW, conv, tol, maxiter, computeobj, anls_alg);
	W_buffer{label_split_pos} = W_buffer_one;
	H_buffer{label_split_pos} = H_buffer_one;
	priority(label_split_pos) = priority_one;
	
	priorities{i} = priority;
	fprintf('%d leaf nodes generated, highest priority %g\n', i+1, max(priority));
end

%--------------------------------------

function [cluster, W_buffer_one, H_buffer_one, priority_one] = trial_split(cluster, trial_allowance, unbalanced, priority, X, subset, new_label, W_parent, scoring, WHinit, random_run, init_used, vec_norm, normW, conv, tol, maxiter, computeobj, anls_alg)
...
while trial < trial_allowance
	[cluster_subset, W_buffer_one, H_buffer_one, priority_one] = potential_split(X, subset, new_label, W_parent, scoring, WHinit, random_run, init_used, vec_norm, normW, conv, tol, maxiter, computeobj, anls_alg);
	if priority_one < 0
		break;
	end
	...
	if min(length_cluster1, length_cluster2) < unbalanced * length(cluster_subset)
		...
		[cluster_subset_small, W_buffer_one_small, H_buffer_one_small, priority_one_small] = potential_split(X, subset_small, new_label, W_buffer_one(:, idx_small), scoring, WHinit, 1, init_used, vec_norm, normW, conv, tol, maxiter, computeobj, anls_alg);
...
function [cluster_subset, W_buffer_one, H_buffer_one, priority_one] = potential_split(X, subset, new_label, W_parent, scoring, WHinit, random_run, init_used, vec_norm, normW, conv, tol, maxiter, computeobj, anls_alg)
...
if length(subset) <= 3
	...
	obj = 1e308;
	for run = 1 : random_run
		[temp_W, temp_H] = nmfsh_comb_rank2(X_subset, 2, WHinit{1, 1, run}.Winit(term_subset, 2*(init_used-1)+1:2*init_used), WHinit{1, 1, run}.Hinit(2*(init_used-1)+1:2*init_used, subset), vec_norm, normW, conv, tol, maxiter, anls_alg);
		if computeobj
			temp_obj = obj_compute(X_subset, temp_W, temp_H);
		else
			temp_obj = 0;
...
function obj = obj_compute(X, W, H)

obj = norm(X, 'fro')^2 - 2 * trace(W' * (X * H')) + trace((W' * W) * (H * H'));
